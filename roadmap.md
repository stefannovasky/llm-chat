...
- [x] 7.2 `/resume` — list and re-enter previous conversations (cost is rehydrated from `Message.Cost` set in 6.3; no refetch via `/api/v1/generation`)
  - Picker UX modeled on `/model` (bubbles list, filter via `/`, esc/ctrl+c close, enter select). Items show title, local-time `updated_at`, message count, aggregated `$cost`. Blocked while streaming/compacting. Selection calls `sessions.Load` and rehydrates: `conversation.Messages` replaced wholesale (system prompt from the file, not re-seeded), UI scrollback rebuilt from domain (compacted messages + synthetic summary included — `CompactedAt` filter keeps LLM requests correct), `sessionID`/`sessionCreatedAt` updated so subsequent autosaves overwrite the same file, active model set to the last assistant message's `Model` (falls back to current if absent). Empty directory / no sessions → dimmed "no saved sessions" message. Known limitation: resuming discards any in-memory unsaved turn from the current session — not an issue in practice because autosave runs after every completed turn.
